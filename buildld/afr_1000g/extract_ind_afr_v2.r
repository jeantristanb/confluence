...
bimfile=read.table('1000g_plk.bim')
...
info_ind<-info_ind[info_ind$Individual.ID %in% bimfile$V1,]
...
write.table(info_ind_afr[!balise_other,c('Individual.ID','Individual.ID')], file='list_afr_noappar_v2.ind',sep="\t",row.names=F, col.names=F,quote=F)
